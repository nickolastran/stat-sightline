...
Managers are deliberately left out — a manager has no player page to put a
ballot on.
"""
...
def ballot(slug: str) -> list[dict] | None:
    ...
    for table in re.findall(r"<table.*?</table>", html, re.S):
        ...
        firsts, points = head.index("1st"), head.index("Points")
        out = []
        for r in rows[1:]:
        ...
        for lg in ("al", "nl"):
            for slug_part, (suffix, first_pts) in AWARDS.items():
                ...
                for i, l in enumerate(lines):
                    ...
                    pid = match(l, roster) or by_surname(l, everyone)
                    if pid is None:
